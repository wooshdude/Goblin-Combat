...
@client.event
async def on_ready():
    path_of_the_directory = 'goblins/players'
    ext = ('.json')
    for files in os.listdir(path_of_the_directory):
        if files.endswith(ext):
            files = files.replace(ext, "", 1)
            print(files)

            player = client.get_user(int(files))
            addToDict(player)
        else:
            continue

    print('goblin combat ready')



# check if player has been assigned a goblin
async def testPlayer(userID, ctx):
    if exists(f"goblins/players/{userID}.json"):
        print('all good')
        return True   
    else:     
...
class Factions(discord.ui.View):
    ...
    async def cave(self, interaction: discord.Interaction, button: discord.ui.Button):
        player = Player(interaction.user, 'cave', items['unarmed'], items['unarmored'])
    ...
    async def desert(self, interaction: discord.Interaction, button: discord.ui.Button):
        player = Player(interaction.user, 'desert', items['unarmed'], items['unarmored'])
    ...
    async def forest(self, interaction: discord.Interaction, button: discord.ui.Button):
        player = Player(interaction.user, 'forest', items['unarmed'], items['unarmored'])
    ...
    async def jungle(self, interaction: discord.Interaction, button: discord.ui.Button):
        player = Player(interaction.user, 'jungle', items['unarmed'], items['unarmored'])
    ...
    async def beach(self, interaction: discord.Interaction, button: discord.ui.Button):
        player = Player(interaction.user, "beach", items['unarmed'], items['unarmored'])
    ...
    async def plains(self, interaction: discord.Interaction, button: discord.ui.Button):
        player = Player(interaction.user, 'plains', items['unarmed'], items['unarmored'])
    ...
    async def snow(self, interaction: discord.Interaction, button: discord.ui.Button):
        player = Player(interaction.user, 'snow', items['unarmed'], items['unarmored'])
...
async def writeToFile(player):
    ...
    if not exists(f'goblins/players/{player.user.id}.json'):
        open_file = open(f'goblins/players/{player.user.id}.json', 'a')
...
async def test(ctx):
    if await testPlayer(ctx.author.id, ctx):
        em = discord.Embed(title=f"{ctx.author.id}'s Goblin Stats")

        f = open(f"goblins/players/{ctx.author.id}.json", "r")
        data = json.load(f)

        em.add_field(name="Class", value=players[ctx.author.id].faction)
        em.add_field(name="Level", value=players[ctx.author.id].level)
        em.add_field(name="HP", value=players[ctx.author.id].hit_points)
        em.add_field(name ="Weapon", value=players[ctx.author.id].weapon)
...
def addToDict(player):
    if player.id not in players.keys():
        f = open(f'goblins/players/{player.id}.json')
        data = json.load(f)
        players[player.id] = Player(player, data['faction'], items[data['weapon'].lower()], items[data['armor'].lower()])
    else:
...
async def profile(ctx):
    if await testPlayer(ctx.author.id, ctx):
        ...
        f = open(f"goblins/players/{ctx.author.id}.json", "r")
        data = json.load(f)
        j = open(f"goblins/factions/{data['faction']}.json")
        factionData = json.load(j)
        

        em.add_field(name="Class", value=factionData['name'])
        em.add_field(name="Level", value=data['level'])
        em.add_field(name="HP", value=data['hit_points'])
        em.add_field(name ="Weapon", value=data['weapon'])
