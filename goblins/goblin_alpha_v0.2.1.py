...
from draw_profile import drawProfile
...
class BossButtons(discord.ui.View):
    ...
    async def menu1(self, interaction: discord.Interaction, button: discord.ui.Button):
        if testPlayer and dungeoneers[interaction.user.id] and not dungeoneers[interaction.user.id].testHealth():
            currentBoss.health -= players[interaction.user.id].attack
    ...
    async def menu2(self, interaction: discord.Interaction, button: discord.ui.Button):
        if testPlayer and dungeoneers[interaction.user.id] and not dungeoneers[interaction.user.id].testHealth():
            ...
            with open(f"factions/{players[self.usr.id].faction['id']}.json") as f:
                faction = json.load(f)
            currentBoss.health -= int(faction['spells'][0]['damage']) + dungeoneers[interaction.user.id].magic
            for i in dungeoneers:
                dungeoneers[i].heal(int(faction['spells'][0]['healing']))
            await testBossHealth()
    ...
    async def menu3(self, interaction: discord.Interaction, button: discord.ui.Button):
        if testPlayer and dungeoneers[interaction.user.id] and not dungeoneers[interaction.user.id].testHealth():
            ...
            with open(f"factions/{players[self.usr.id].faction['id']}.json") as f:
                faction = json.load(f)
            currentBoss.health -= int(faction['spells'][1]['damage']) + dungeoneers[interaction.user.id].magic
            for i in dungeoneers:
                dungeoneers[i].heal(int(faction['spells'][1]['healing']))
            await testBossHealth()
...
async def bossAttack():
    try:
        ...
        dungeoneers[randPlayer].hit_points -= currentBoss.attack
        em, img = await bossFight()
...
# prospero
# client.run("OTE4Nzg0MzI3NzY3MTkxNTcy.GDJgQn.K3jTzQc2eyeSHxNM86nSNaQGFTm2TDzz8_6fXg")
